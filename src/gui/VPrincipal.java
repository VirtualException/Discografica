...
public class VPrincipal extends javax.swing.JFrame {
    ...
    private void botonPremiosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonPremiosActionPerformed
        ...
        this.tablaPremios.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) { 
                int filaSeleccionada = tablaPremios.getSelectedRow();
                if (filaSeleccionada != -1) {
            
                   String nombre = tablaPremios.getValueAt(filaSeleccionada, 0).toString(); 
                   String institucion = tablaPremios.getValueAt(filaSeleccionada, 1).toString();    
                   premiosNombre.setText(nombre);
                   premiosInstitucion.setText(institucion);
                }
            }
        });
        gbc.gridx = 3;
        ...
        this.botonBuscarPremios.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            String nombre = premiosNombre.getText();
            String institucion = premiosInstitucion.getText();
            buscarPremio(nombre, institucion);
        }
        });
        ...
        this.botonBorrarPremios.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            borrarPremio();
        }
        });
        ...
        this.botonEditarPremios.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            editarPremio();
        }
        });
        ...
        this.botonNuevoPremio.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            nuevoPremio();
        }
        });
...
private void buscarPremio(String nombre, String institucion) {
    ModeloTablaPremios m;
        m=(ModeloTablaPremios) tablaPremios.getModel();
        m.setFilas(fa.obtenerPremio(nombre, institucion));
        if (m.getRowCount() > 0) {
            tablaPremios.setRowSelectionInterval(0, 0);
            premiosNombre.setText(m.obtenerPremiacion(tablaPremios.getSelectedRow()).getPremio().getNombre());
            premiosInstitucion.setText(m.obtenerPremiacion(tablaPremios.getSelectedRow()).getPremio().getInstitucion());
            botonBorrarPremios.setEnabled(true);
            botonEditarPremios.setEnabled(true);
            
        }
        else {
            botonBorrarPremios.setEnabled(false);
            botonEditarTrabajadores.setEnabled(false);
        }
}
private void nuevoPremio() {
    VGestionPremios vp = new VGestionPremios(this, true, this.fa);
    vp.setVisible(true);
}
private void editarPremio(){
    String nombre = premiosNombre.getText();
    String institucion = premiosInstitucion.getText();
    List<Premiacion> listapremios = fa.obtenerPremio(nombre, institucion);
    for (Premiacion p: listapremios){
        VGestionPremios vp = new VGestionPremios(this, true, this.fa, p);
        vp.setVisible(true);
    }
}
private void borrarPremio(){
    String nombre = premiosNombre.getText();
    String institucion = premiosInstitucion.getText();
    fa.borrarPremio(nombre, institucion);
}
}